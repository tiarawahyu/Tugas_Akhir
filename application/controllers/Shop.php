    ...
    public function checkout($action = '')
    {
        ...
        switch ($action)
        {
            default :
                ...
                $items = [];
                $total_item_as_unit = 0;
                foreach ($this->cart->contents() as $item)
                {
                    $total_item_as_unit += $item['qty'];
                ...
                $kota = $this->db->query("SELECT * FROM ro_city")->result();

                $params['kota'] = $kota;
                $params['customer'] = $this->customer->data();
                $params['as_user'] = $this->customer->as_user()[0]->email;
                ...
                $selected_kota = "--pilih--";
                $ongkir = [];
                if($this->input->post('selected_kota')){
                    $selected_kota = $this->input->post('selected_kota');
                    if($selected_kota != "--pilih--"){
                        $ongkir = $this->ro_cost("398", $selected_kota, 1000 * $total_item_as_unit, "jne")["results"][0];
                    }
                }
                $params['selected_kota'] = $selected_kota;
                $params['ongkir'] = $ongkir;
                // var_dump($total_item_as_unit);
                // var_dump($ongkir);
                // var_dump($selected_kota);
            ...
            case 'order' :
                ...
                $pengiriman_addon = $this->input->post('pengiriman_addon');
                $pengiriman_addon = json_decode($pengiriman_addon);
                ...
                $delivery_data = array(
                    ...
                    'kota' => $kota[0],
                    'kode_pos' => $kota[1],
                    'pengiriman_value' => (int)$pengiriman_addon->value,
                    'pengiriman_service' => $pengiriman_addon->service,
                    'pengiriman_description' => $pengiriman_addon->description,
                    'pengiriman_etd' => $pengiriman_addon->etd,
                    'note' => $note
                );
                ...
                redirect('Payment/checkout_order/'. $order);
            break;
        }

    }

    public function ro_cost($origin, $destination, $weight, $courier){
        $curl = curl_init();
        curl_setopt_array($curl, array(
            ...
            CURLOPT_HTTPHEADER => array(
                ...
                "key: " . "1c0f510e542140c008e2b49d73e5f645",
            ),
        ));
