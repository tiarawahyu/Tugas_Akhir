...
class Order_model extends CI_Model {
    ...
    public function get_all_orders_filter($limit, $start, $periode, $status)
    {
        $qry = "
            SELECT o.id, o.order_number, o.order_date, o.order_status, o.payment_method, o.total_price, o.total_items, c.name AS coupon, cu.name AS customer
            FROM orders o
        ...
        ";
        if($status != "_ALL_"){
            $qry .= " WHERE o.order_status = '$status'";
        }
        $orders = $this->db->query($qry . "
            ORDER BY o.order_date DESC
            LIMIT $start, $limit
        ");
        ...
        foreach($orders as $k => $row){
            ...
            $flag_unset = false;
            if($periode == "YEAR" && $order_date[0] != $year){
                $flag_unset = true;
            }
            if($periode == "MONTH" && $order_date[0]."-".$order_date[1] != $month){
                $flag_unset = true;
            }
            if($periode == "DAY" && $order_date[0]."-".$order_date[1]."-".$order_date_day[0] != $day){
                $flag_unset = true;
            }
            if($flag_unset){
                unset($orders[$k]);
